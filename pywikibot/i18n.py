...
def translate(code, xdict, parameters=None, fallback=False):
    ...
    @param fallback: Try an alternate language code. If it's iterable
        it'll also try those entries and choose the first match. If it's
        a Wikibase item, it'll try to load local page title from here.
    ...
    if not isinstance(xdict, dict):
        ...
        codes = [code]
        if fallback is True:
            codes += _altlang(code) + ['_default',
                                       'en'] + [list(xdict.keys())[0]]
        ...
        for code in codes:
            ...
            if isinstance(fallback, UnicodeType):
                ...
                # but when fallback was True before 65518573d2b0, it did
                # just that. When False it did just return None. It's
                # now also returning None in the new iterable mode.
                return None
